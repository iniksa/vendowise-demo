...
# ---------------------------
def login():
    st.title("\U0001F511 VendoWise Login")
...
def inventory_dashboard(inv_data):
    st.title("\U0001F4E6 Inventory Risk Dashboard")
    ...
    st.subheader("\U0001F4CA Inventory Risk Summary")
    ...
    st.subheader("\U0001F4C9 Inventory Risk Classification Chart")
...
def vendor_dashboard(vendor_data):
    tab1, tab2 = st.tabs(["\U0001F4C8 Vendor Performance", "\U0001F9EE Vendor PO Risk Simulation"])

    with tab1:
        st.subheader("\U0001F69A Vendor Delivery Performance")
        ...
        st.dataframe(vendor_data.style.applymap(lambda val: "background-color: red" if val is False else "", subset=["on_time"]))

        st.subheader("\U0001F4E6 Rejection & Freight Overview")
        ...
        st.dataframe(vendor_data[["vendor_name", "item_code", "rejection_rate (%)", "freight_cost", "location_risk"]]
            .style.applymap(lambda val: "background-color: orange" if isinstance(val, (int, float)) and val > config["max_location_risk"] else "", subset=["location_risk"]))

        st.subheader("\U0001F4CA Rejection Rate Chart")
...
def main():
    ...
    if choice == "Inventory Dashboard":
        if inventory_data is not None:
            ...
            st.warning("Please upload or select sample inventory data.")
    elif choice == "Vendor Dashboard":
        if vendor_data is not None:
            vendor_dashboard(vendor_data)
        else:
            st.warning("Please upload or select sample vendor data.")
    elif choice == "Logout":
        st.session_state["logged_in"] = False
        st.experimental_rerun()
